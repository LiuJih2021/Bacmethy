...
awk -F'\t' '{print $2"\t"$3"\t"$4"\t"$5"\t"$8"\t"$7}' $1 |cut -f2 -d":" |sed 's/\([0-9]\+\)-\([0-9]\+\)/\1\t\2/g'  > fimo_processed.txt
...
}
{
  for (i in fimo) {
    split(i, arr, SUBSEP)
    key = arr[1]
    if (key == $4 || key == $5) print $0 "\t" fimo[i]
  }
}' fimo_processed.txt $2_methylationGene.txt > final_output_temp_1.txt
awk '
NR==FNR {
  fimo[$2, NR]=$0
  next
